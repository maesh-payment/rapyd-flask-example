...
@app.route("/rapyd", methods=['GET','POST'])
def rapyd():
    checkout_body = {}
    checkout_body["amount"] = 145
    checkout_body["currency"] = "SGD"
    checkout_body["country"] = "SG"
    checkout_body["complete_checkout_url"] = "http://example.com/complete"
    checkout_body["payment_method_types_include"] = ["sg_paynow_bank", "sg_ocbcpao_bank"]

    results = make_rapyd_request(method="post", path="/v1/checkout", body=checkout_body)

    print(results)

    return redirect(results["data"]["redirect_url"])
